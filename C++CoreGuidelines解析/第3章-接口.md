...
> 事实上原书给的上面这段代码是有问题的，无法在 `msvc` 通过编译，这里使用的是 **`0`** 做初始值，有窄化转换，msvc 使用的是 `{}` 初始化，检测到了，于是编译错误。严重怀疑他们根本没测试过 `msvc` 编译器。 这里应该把 0 换成 `Oull` （基于当前 64 位环境），或者标准够高使用 `0uz`，再或者直接 `std::size_t{0}`。
